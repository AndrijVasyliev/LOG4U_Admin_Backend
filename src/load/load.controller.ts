...
import { CreateLoadValidation, UpdateLoadValidation, loadQueryParamsSchema } from './load.validation';
...
export class LoadController {
    ...
    @Get()
    async getLoads(
        @Query(new QueryParamsPipe<LoadQuerySearch>(loadQueryParamsSchema)) loadQuery: LoadQuery,
    ...
    @Post()
    async createLoad(
        @Body(new BodyValidationPipe(CreateLoadValidation)) createLoadBodyDto: CreateLoadDto,
    ): Promise<LoadResultDto> {
    ...
    async updateLoad(
        @Param('loadId', MongoObjectIdPipe) loadId: string,
        @Body(new BodyValidationPipe(UpdateLoadValidation)) updateLoadBodyDto: UpdateLoadDto,
    ): Promise<LoadResultDto> {
